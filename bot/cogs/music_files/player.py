...
class Player(wavelink.Player):
    ...
    async def add_track(self,track: Track,text_channel: discord.TextChannel):
        ...
        if not hasattr(self,"queue_message"):
            self.queue_message = await text_channel.send('Loading queue...')
        else:
        ...
        if not hasattr(self,"message"):
            self.message = await text_channel.send('Loading player...')
    ...
    async def get_queue_embed(self,page = 1):
        ...
        embed = discord.Embed(
            title= "Upcoming", 
            description=f"Total Track(s) `{self.total_track() - self._now_playing -1}`" , 
            color=0x4287F5)

        if self.total_track() - self._now_playing -1 == 0:
            embed.add_field(name="No more song in the queue", value = "⠀")
        tot = (page-1)*5 + 1 
        for i in range(self._now_playing+tot,self.total_track()):
            ...
            embed.add_field(
                ...
                value = f"Picked by - {track.picked_by.display_name}",
                inline=False) 
            tot = tot + 1 
    ...
    class View(ui.View):
        ...
        async def on_error(self, error, item, interaction:discord.Interaction):
            if isinstance(error, Error.NotInCorrectVoiceChannel):
                msg = await interaction.channel.send(f"{interaction.user.mention} You need to be in voice channel: `{self.player.channel}` to do that")
    ...
    def get_view(self):
        ...
        async def loop(button,interaction):
            ...
            embed = self.current_track().get_link_embed('Now Playing' , self.Loop.name[self.loop])
